...
class ColumnLeftComponent extends Component {
    componentDidMount(){
        this.props.getPostsSlugTag(this.props.slugTag);
    }
        ...
        if(post.view_slider === false && post.positionTop === false)
            return (
                <Link to={'/post/'+post.slug.NameSlug } style={{ color: 'black', textDecoration: 'none' }}>
                    ...
                    <img src={ post.thumbnail } alt=""  width="80px;" style={{ float: "left" }}/>
                    <span>
                        <h4>{ post.title }</h4>
    ...
    render() {
        ...
        return(
            ...
            //             <div>
            //                 { this.props.postsSlugTag.map(post => this.getListPostFeature(post))}
            //             </div>
            //             </div>
            //             <div className="col-sm-3" style={{ borderLeft: "1px solid #e6e6e6" }}>
            //                 <h2 style={{ borderBottom: "3px solid #2e439c" }}>Tin chọn lọc</h2>
            //                 { this.props.postsSlugTag.map(post => this.getListPostFilter(post))}
            //             </div>
            //     </div>
            //     <div className="row" style={{ borderTop: "1px solid #e6e6e6" }}>
            //             <div className="col-sm-12">
            //                 { this.props.postFollowCategory.map(post => this.getListPost(post))}
            //             </div>
            //        </div>
            // </div>
